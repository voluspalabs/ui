...
// biome-ignore lint/style/noMagicNumbers: Cookie max age calculation (7 days in seconds)
...
function SidebarProvider({
  ...
  const setOpen = useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // biome-ignore lint/suspicious/noDocumentCookie: Direct cookie access is required for sidebar state persistence
...
// Random width calculation constants for skeleton variation (50% to 90%)
...
function SidebarMenuSkeleton({
  ...
  const [width] = useState(
    () =>
      `${Math.floor(Math.random() * SKELETON_WIDTH_RANGE_PERCENT) + SKELETON_MIN_WIDTH_PERCENT}%`,
  )
